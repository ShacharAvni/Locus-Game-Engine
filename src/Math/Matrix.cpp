...
bool Matrix<ScalarType>::IsRowAllZero(unsigned int row, unsigned int upToColumn) const
{
   ...
   {
      if (Float::FNotZero<ScalarType>(At(row, col)))
      {
         return false;
      }
   }
...
{
   bool pivotInPlace = Float::FNotZero<ScalarType>(At(col, col));
   ...
   if (pivotInPlace)
   {
      ...
      if (reduce)
      {
         for (unsigned int row = 0; row < rowOfPivot; ++row)
         {
            ...
            if (Float::FNotZero<ScalarType>(valueToMakeZero))
            {
...
void Matrix<ScalarType>::ReducePivot(unsigned int col)
{
   ...
   if (Float::FNotZero<ScalarType>(pivot))
   {
